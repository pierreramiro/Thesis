#ifndef _OUSTER_RECONSTRUCTION_H
#define _OUSTER_RECONSTRUCTION_H	1
#include "math.h"
#include "time.h"
...
void TwoandTri_Donut_Fill(double* Point_Cloud,unsigned int* TwoDF,unsigned int* TriDF,unsigned int* MidDF){
    ...
    unsigned int v0_mid,v1_mid,v2_mid,T_mid[3],v0_lim,v1_lim,index;//new_v_init,v2_lim
...
void Generate_surface(double* Point_Cloud,unsigned int* T,unsigned int *pointer_n_triangles){
    ...
    unsigned int* T_Sphere=(unsigned int*)malloc(n_total_triangles * 3 *sizeof(unsigned int));
    ...
    for (unsigned int i = 0; i < n_total_points; i++){
        xp=Sphere_Cloud[i*3+0];
        yp=Sphere_Cloud[i*3+1];
        zp=Sphere_Cloud[i*3+2];
        if ((xp==0)&&(yp==0)&&(zp==0)){
            Point_Cloud[i*3+0]=0;
            Point_Cloud[i*3+1]=0;
            Point_Cloud[i*3+2]=0;
