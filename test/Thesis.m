function TotalTriangleMesh=Thesis(rot_angle)
%clearvars except rott_angle;
close all
...
%rango elevación
%n_azimuths
%Re ha realizado un for con valores desde 25 hasta 160°
...
%Angle rotation,n° donuts,angular velocity,time per scan.
    if nargin==0
        ...
        if rot_angle == -35*pi/180
            fprintf("Ocurrirá un error en la reconstrucción\n")
        elseif rot_angle >beam_altitude_angles(end)
            fprintf("OJO. El ángulo de rotación es menor que el ángulo de elevación del LIDAR, considerar otro ángulo\n");
        elseif rot_angle <-pi
            %Establecer aqui una función error.
            error("OJO. El LIDAR no debería girar más de 180°")
        end
    end
    n_donuts=1;
...
%%
figure(3)
temp=[OneDonutFill;TwoDonutFill;TriDonutFill];
